  ...
  <form method="post" action="${pageContext.request.contextPath}/reservas">
    ...
    <div class="mb-3">
      ...
      <select name="clienteId" class="form-select" required>
        ...
        %>
          <option value="<%= c.getId() %>" <%= r != null && r.getClienteId() == c.getId() ? "selected" : "" %>><%= c.getNombre() %> - <%= c.getEmail() %></option>
        <%
            }
          }
        %>
    ...
    <div class="mb-3">
      ...
      <select name="paqueteId" class="form-select" required>
        ...
        %>
          <option value="<%= p.getId() %>" <%= r != null && r.getPaqueteId() == p.getId() ? "selected" : "" %>><%= p.getTitulo() %> - $<%= p.getPrecio() %></option>
        <%
            }
          }
        %>
    ...
    <div class="row">
      <div class="col-md-4 mb-3"><label class="form-label">Fecha</label><input type="date" name="fechaReserva" class="form-control" value="<%= r != null && r.getFechaReserva() != null ? r.getFechaReserva().toString() : "" %>" required></div>
      <div class="col-md-4 mb-3"><label class="form-label">Personas</label><input type="number" min="1" name="numeroPersonas" class="form-control" value="<%= r != null ? r.getNumeroPersonas() : 1 %>"></div>
    ...
    <a class="btn btn-secondary mt-2" href="${pageContext.request.contextPath}/reservas">Cancelar</a>
  </form>
</div>
</body>
</html>